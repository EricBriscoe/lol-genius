...
_METHOD_PATTERNS: list[tuple[re.Pattern, str]] = [
    ...
    (
        re.compile(r"/lol/champion-mastery/v4/champion-masteries/by-puuid/.+/top"),
        "mastery",
    ),
...
def _resolve_method_path(path: str) -> str | None:
...
def resolve_method(url: str) -> str | None:
    return _resolve_method_path(urlparse(url).path)
...
class RateLimiter:
    def __init__(
        ...
        self.timestamps: dict[int, deque[float]] = {w: deque() for _, w in self.buckets}
        ...
        for i, wait in enumerate(AUTH_BACKOFF_STEPS):
            ...
            try:
                probe = self.client.get(
                    "https://na1.api.riotgames.com/lol/league/v4/entries/RANKED_SOLO_5x5/IRON/I?page=1"
                )
