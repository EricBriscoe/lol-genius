...
export async function loadModel(modelDir: string, modelType = "live", force = false): Promise<void> {
  ...
  if (!existsSync(namesPath)) {
    throw new Error(`Feature names file not found at ${namesPath}; the model directory may be incomplete`);
  }
  ...
  if (meta && "shape" in meta && Array.isArray(meta.shape) && typeof meta.shape[1] === "number") {
    ...
    if (featureNames.length !== expectedDim) {
      throw new Error(
        `${modelType} model mismatch: feature_names.json has ${featureNames.length} features but model.onnx expects ${expectedDim}. The model release is invalid. Retrain the model or fix the release assets.`
      );
    }
  }
